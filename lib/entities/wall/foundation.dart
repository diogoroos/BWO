...
class Foundation {
  ...
  Paint p = Paint();
  final TextPaint _txt10 =
      TextPaint(style: TextStyle(fontSize: 10.0, color: Colors.blueGrey[700], fontFamily: "Blocktopia"));
  ...
  dynamic fromListToObject() {
    ...
    furnitureList.forEach((key, value) {
      finalObject["furnitures"] = [...finalObject["furnitures"], value.toObject()];
    });
  ...
  bool isInsideFoundation(double posX, double posY, {double wPoint = 0, double hPoint = 0}) {
    return posX >= left && posX + wPoint < (left + width) && posY >= top && posY + hPoint <= (top + height);
  }
  ...
  void switchWallHeightAll({bool isBuildingMode = false}) {
    ...
    _isPreviousInsideFoundation = isInsideFoundation(_player.posX.toDouble(), _player.posY.toDouble());
  }

  void switchWallHeight(Wall wall, {bool isBuildingMode = false}) {
    var isCurrentInsideFoundation = isInsideFoundation(_player.posX.toDouble(), _player.posY.toDouble());
