...
class Furniture extends Entity {
  ...
  Furniture(double newPosX, double newPosY, this.width, this.height, this.imageId)
  ...
  void draw(Canvas c) {
    ...
    // Or originalSize
    var pivot = Offset((zoom * 16) / 2, (currentSprite.srcSize.y * 2) - height + 16);

    currentSprite.render(c,
        position: Vector2(x - pivot.dx, y - pivot.dy - z), size: Vector2.all(SpriteController.spriteSize * 2));
  ...
  bool isInside(int pointX, int pointY) {
    return pointX >= posX && pointX < (posX + width ~/ 16) && pointY >= posY - 1 && pointY < (posY - 1 + height ~/ 16);
  }

  bool isIntersecting(double pointX, double pointY, double wPoint, double hPoint) {
    var r1 = Rectangle(
  ...
  dynamic toObject() {
    return {'id': imageId, 'x': posX, 'y': posY - 1, 'w': width / 16, 'h': height / 16};
  }
}
