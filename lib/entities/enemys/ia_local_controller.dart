...
import '../../entities/enemys/ia_controller.dart';
...
class IALocalController extends IAController {
  ...
  void patrolArea() {
    ...
    if (GameController.time > _newPatrolAreaDelay) {
      ...
      _destPoint = Offset(self.x + Random().nextInt(patrolAreaRange) - patrolAreaRange / 2,
  ...
  void searchForTargetsEntity() {
    ...
    for (var entityA in self.map.entitysOnViewport) {
      if (entityA != self && entityA.isActive && entityA.status.isAlive() && entityA is Player) {
        var distance = (Offset(self.x, self.y) - Offset(entityA.x, entityA.y)).distance;

        if (distance < seeTargetDistanceMin) {
          target = entityA;
        }
        var targetSpeed = (entityA.xSpeed + entityA.ySpeed).abs();
  ...
  void _attackingTarget() {
    ...
    var distanceToTarget = (Offset(self.x, self.y) - Offset(target.x, target.y)).distance;

    if (distanceToTarget > _loseTargetDistance || target.status.isAlive() == false) {
      target = null;
      return;
    }

    var totalSpeed = self.xSpeed + self.ySpeed;
    if (GameController.time > _attackSpeedDelay && distanceToTarget < attackDistance && totalSpeed < 1) {
      _attackSpeedDelay = GameController.time + attackFrequencyInSec;

      self.currentSprite.setDirection(Offset(target.x, target.y), Offset(self.x, self.y));
