...
import '../../entities/equipment_controller.dart';
import '../../entities/items/items.dart';
import '../../entities/player/input_controller.dart';
import '../../entities/player/player_actions.dart';
import '../../entities/player/player_network.dart';
...
class Player extends Entity implements OnAnimationEnd {
  final TextPaint _text = TextPaint(style: TextStyle(fontSize: 12.0, color: Colors.white, fontFamily: "Blocktopia"));
  ...
  Player(double x, double y, this._map, String myName, String myId, this.sceneObject,
  ...
  void draw(Canvas c) {
    ...
    if (_inputController != null) {
      maxWalkSpeed = (_inputController.maxAngle * _inputController.speedMultiplier);
    }
    ...
    if (currentSprite != null) {
      ...
      equipmentController?.draw(c, animSpeed, stopAnimWhenIdle: stopAnimWhenIdle);
    }
    //debugDraw(c);
  ...
  void die(Canvas c) {
    if (status.isAlive() == false) {
      isActive = false;
      _deathSprite?.render(c, position: Vector2(x - 16, y - 32), size: Vector2.all(SpriteController.spriteSize * 2));
  ...
  void _loadSprites() {
    ...
    walkSprites = SpriteController("$spriteFolder/walk", _viewPort, _pivot, _scale, _gradeSize, _framesCount, this);
    attackSprites =
        SpriteController("$spriteFolder/attack", _viewPort, _pivot, _scale, Offset(5, 1), _framesCount, this);
