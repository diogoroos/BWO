...
class PlayerNetwork {
  ...
  void _moveLerpPosition() {
    ...
    if (distance > 250) {
      ...
      if (distance > 8) {
        ...
        player.x = lerpDouble(player.x, targetPos.dx, GameController.deltaTime * lerpSpeed);
        player.y = lerpDouble(player.y, targetPos.dy, GameController.deltaTime * lerpSpeed);
      }
    }
  }
  ...
  void sendHitTree(int x, int y, int damage) {
    ...
    // GameScene.serverController.hitTree(x, y, damage);
    // _tControl.onTreeUpdate
