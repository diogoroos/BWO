  ...
  void registerAccelerometer() {
    accelerometerEvents.listen((event) {
      ...
      if (TapState.isTapingRight() && TapState.isTapingBottom()) {
        player.xSpeed = (eventX * accelerationSpeed).clamp(-maxAngle, maxAngle).toDouble() * speedMultiplier;
        player.ySpeed =
            ((eventY - defaultY) * -accelerationSpeed).clamp(-maxAngle, maxAngle).toDouble() * speedMultiplier;
      } else {
        previousY = eventY;
        player.xSpeed = 0;
        player.ySpeed = 0;
      ...
      if (player.ySpeed.abs() + player.xSpeed.abs() < 0.6 || player.playerActions.isDoingAction) {
        player.xSpeed = 0;
        player.ySpeed = 0;
      }
    });
