...
class PlayerHUD extends UIElement {
  ...
  final TextPaint _txt14 = TextPaint(style: TextStyle(fontSize: 14.0, color: Colors.white, fontFamily: "Blocktopia"));
  final TextPaint _txt10 = TextPaint(style: TextStyle(fontSize: 10.0, color: Colors.white, fontFamily: "Blocktopia"));
  ...
  void draw(Canvas c) {
    ...
    //energy
    drawBar(c, Rect.fromLTWH(10, 40, 100, 8), Color.fromRGBO(229, 184, 46, 1), _player.status.getEnergy(),
        _player.status.getMaxEnergy(), "${_player.status.getEnergy().toStringAsFixed(1)}", _txt10);
    ...
    _txt10.render(c, "${_player.posX}, ${_player.posY}", Vector2(hp.right, hp.top - 1), anchor: Anchor.bottomRight);
  ...
  Rect drawBar(
      Canvas c, Rect barRect, Color barColor, double currentVal, double maxVal, String textValue, TextPaint txt) {
    var bar2 = Rect.fromLTWH(barRect.left + 2, barRect.top - 2, barRect.width - 4, barRect.height + 4);
    ...
    var rBar2 = Rect.fromLTWH(barRect.left + 2, barRect.top - 2, barLar - 4, barRect.height + 4);
  ...
  void drawIcon(Canvas c, Rect r, Sprite back, Sprite front, double currentVal, double maxVal) {
    back.renderRect(c, r);

    var barHeight = (((currentVal * 100 / (maxVal)) / 100) * r.height);

    c.save();
    c.clipRect(Rect.fromLTWH(r.left, r.top + (r.height - barHeight), r.width, barHeight));
